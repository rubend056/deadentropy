...
const config = (env, argv) => {
	...
	const to_define = Object.fromEntries([
		...[...Object.entries(_to_define), ...Object.entries(process.env)].map(([k, v]) => [
			'process.env.' + k + '__S',
			JSON.stringify(v),
		]),
	...
	const common: Configuration = {
			...
			rules: [
				...
				// },
				{
					...
					use: [
						...
						{
							loader: resolve(__dirname, './db/couchdb/couchjs-design.js'),
						},
					],
				},
				{
				...
				},
				{
					...
					use: {
						loader: 'file-loader',
						options: {
							outputPath: join(assert_value(process.env.APP_PATH), 'assets'),
							publicPath: 'assets'
						},
					},
				},
		...
		resolve: {
			extensions: [
				...
				...['.module', '.mod', '.m', ''].flatMap((a) => ['.css', '.scss', '.sass'].map((b) => a + b)),
